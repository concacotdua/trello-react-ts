...
const Card: FC<CardType> = ({ _id, attachments, columnId, comments, cover, memberIds, title, FE_placeholderCard }: CardType) => {
  ...
  return (
    <div
      className={cn('group relative rounded-md border bg-card p-2 md:p-3', 'shadow-sm', FE_placeholderCard ? 'hidden' : 'block')}
      ref={setNodeRef}
      style={dndCardStyle}
      {...attributes}
      {...listeners}
              ...
              {memberIds.map((member, index) => (
                <Tooltip key={index}>
                  <TooltipTrigger asChild>
                    <Avatar className='h-5 w-5 select-none border-2 border-background md:h-6 md:w-6' draggable={false}>
                      <AvatarImage src={`https://avatar.vercel.sh/${member}`} draggable={false} />
                      <AvatarFallback className='text-[10px] md:text-xs'>{member.slice(0, 2).toUpperCase()}</AvatarFallback>
                    </Avatar>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>{member}</p>
